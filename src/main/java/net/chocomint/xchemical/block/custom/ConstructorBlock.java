...
import net.chocomint.xchemical.block.entity.ConstructorBlockEntity;
...
public class ConstructorBlock extends BlockWithEntity implements BlockEntityProvider {
	...
	public void onStateReplaced(BlockState state, World world, BlockPos pos, BlockState newState, boolean moved) {
		if (state.getBlock() != newState.getBlock()) {
			...
			if (blockEntity instanceof ConstructorBlockEntity) {
				ItemScatterer.spawn(world, pos, (ConstructorBlockEntity) blockEntity);
	...
	public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
		return checkType(type, ModBlockEntities.CONSTRUCTOR_BLOCK_ENTITY, ConstructorBlockEntity::tick);
	}
}
